...
public class CreateCatalogItemEndpoint : IEndpoint<IResult, CreateCatalogItemRequest>
{
    ...
    public async Task<IResult> HandleAsync(CreateCatalogItemRequest request)
    {
        ...
        var newItem = new CatalogItem(request.CatalogTypeId, request.CatalogBrandId, request.Description, request.Name, request.Price, request.PictureUri, request.Subtitle);
